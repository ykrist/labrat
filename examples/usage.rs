...
impl IdStr for Inputs {
    fn id_str(&self) -> String {
        format!(
            "IDX{:03}_TW{:06}",
            self.index,
            (self.tw_scale * 1000.0).round() as u64
        )
    }
}
...
impl IdStr for Params {
    fn id_str(&self) -> String {
        self.param_name
            .clone()
            .unwrap_or_else(|| id_from_serialised(self))
    }
}
...
impl Experiment for MyExperiment {
    ...
    fn new_output(inputs: &Inputs, _params: &Params, config: &Self::Config) -> Self::Output {
        Outputs {
            ...
            trace_log: if config.trace_log {
                Some(format!("{}-tracelog.json", inputs.id_str()))
            } else {
                None
            },
        }
...
impl ResourcePolicy for MyExperiment {
    fn time(&self) -> Duration {
        Duration::from_secs(300 + 60 * (self.inputs.index / 10))
    }
    fn memory(&self) -> MemoryAmount {
        MemoryAmount::from_gb(4)
    }
